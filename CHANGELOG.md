...
首个版本。11 个探针，8 个本地模拟场景，15 项端到端验收，15 项模型选择单元测试，
外加一个不装 Python 也能双击运行的 Windows 桌面版。
...
`relaycheck-<版本>-windows-x64.zip`（实测约 14 MB）挂到 Release 页。解压 → 打开
`relaycheck-desktop\` → 双击 `relaycheck-gui.exe`。目标机器不需要装 Python。
...
`relaycheck/__init__.py` 一处。实测把 `__version__` 临时改成 `9.9.9`，构建出来的
...
`_make_output_safe()` **行为未变**（它依旧只放松 `errors`，从不碰 `encoding`），
所以命令行审计本身的行为没有变化。
